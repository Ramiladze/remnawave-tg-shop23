...
2.  **Создайте и настройте файл `.env`:**
    ...
    | `SUBSCRIPTION_BASE_URL` | (Опционально) Базовый URL для динамических ссылок на подписку. | `https://your.domain.com/subscription` |
    </details>
    ...
    Дополнительно можно настроить эксклюзивные цены для конкретных пользователей. 
    Укажите их Telegram user_id через запятую и альтернативные цены в рублях:

    - `EXCLUSIVE_USER_IDS`: Список ID, например `12345,67890`
    - `RUB_PRICE_1_MONTH_EXCLUSIVE`: Цена для 1 месяца (пример: `99`)
    - `RUB_PRICE_3_MONTHS_EXCLUSIVE`: Цена для 3 месяцев (пример: `266`)
    - `RUB_PRICE_6_MONTHS_EXCLUSIVE`: Цена для 6 месяцев (пример: `505`)
    - `RUB_PRICE_12_MONTHS_EXCLUSIVE`: Цена для 12 месяцев (пример: `1070`)

    Поведение:
    - Если для эксклюзивного пользователя задана эксклюзивная цена для периода — она используется.
    - Если какая‑то эксклюзивная цена не задана, для этого периода берётся обычная цена.
    </details>
